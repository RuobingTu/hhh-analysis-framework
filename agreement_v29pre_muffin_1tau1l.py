#!/usr/bin/env python3
"""
Data/MC agreement plots for the v29pre option-92 1tau1l channel (NOT the
1tau1l-hadronic control) with the data-driven Fake-LEPTON background.

Adapted from closure_v29pre_in_v29pre_option92_1tau0l_NHiggs_20bin_split.py
(rendering machinery identical). Method changes:
  - Region: kind_category_FR == 1 (1tau1l at Fake WP), tau1 REQUIRED at the
    analysis WP (VSjet>=8) on BOTH legs -> the fake axis is the LEPTON only.
  - tight  = lep1passAnalysisWP (mvaTTH>0.40 mu / >0.30 e)
    anti-ID = Fakeable-not-tight lepton
  - FakeLep template = data_AR x FF - prompt-lep MC_AR x FF, with
    FF = fr/(1-fr) from the 2017 mT-template-fit map (lepfr_map_2017_final.npz,
    values EMBEDDED below); cone-pT = c*pt*(1+jetRelIso), c=0.756/0.80.
  - MC stack gated on prompt lepton (genPartFlav 1/15/22; conversions folded
    into the prompt groups -> measurement-side convention). Tau leg (genuine or
    fake) stays MC-modeled: this validates the LEPTON axis only.
  - MC weight uses triggerSF_1tau1l_v1 (leptonic OR-formula SF), not the
    hadronic per-filter JetHT SF; no triggerLumiSF (leptonic paths full-lumi).
  - Data = SingleMuon (mu-channel) + SingleElectron (e-channel), routed by
    abs(lep1Id) to kill PD overlap.

Original 1tau0l docstring follows for the shared machinery:
...
OUTDIR = os.environ.get('CLOSURE_OUTDIR',
    '/afs/cern.ch/user/r/rtu/CMSSW_12_5_2/src/hhh-analysis-framework/plots_v29pre_1tau1l_fakelep_20bin_split')
...
REGION_BASE = os.environ.get('REGION_BASE', 'kind_category_FR == 1')
# FAKETAU=1: full double-fake model — add the FakeTau template from the 1tau0l
# fr2d map applied AS-IS (no transfer correction): SR = (lep tight, tau>=8);
#   N_fake = FF_l x (lepAR,tauT) + FF_tau x (lepT,tauAR) - FF_l*FF_tau x (lepAR,tauAR)
# with (prompt-lep && genuine-tau) MC subtracted in every AR, and the MC stack
# gated on prompt-lep && genuine-tau. Validation: SS in-channel ratio = 1.057.
FAKETAU = os.environ.get('FAKETAU', '') == '1'
if FAKETAU and 'CLOSURE_OUTDIR' not in os.environ:
    OUTDIR = OUTDIR + '_withFakeTau'
...
# USE_MUFFIN=1: the fake-tau transfer factor becomes the MUFFIN weight
# (muffin/README.md), measured in 1tau0l and applied here unchanged.  Everything
# else -- regions, fake-lepton treatment, samples, binning -- is untouched.
USE_MUFFIN = os.environ.get('USE_MUFFIN', '') == '1'
MUFFIN_HEADER = os.environ.get('MUFFIN_HEADER', os.path.join(
    THIS_DIR, 'muffin', 'out', 'muffin_poster.h'))
...
NO_ZERO_FILTER_VARS = set([
    ...
    'IndexMaxProb', 'IndexMaxCat',
] + ['%s%s' % (c, p) for p in ('_v28common_ep281', '_v28qcdfrac_ep289')
...
def build_plot_vars():
    ...
    # tau mother-jet variables (FR-axis acceptance gates for the axis reduction)
    pv += [
    ...
    # 1tau1l two-leg variables (sentinel/absent in 1tau0l -> auto-dropped there)
    pv += [
        ('lep1Mt',     'lep_{1} M_{T}(lep,MET) [GeV]', cont(0, 200)),
        ('dzeta',      'D_{#zeta} [GeV]',              cont(-150, 150)),
        ('pzeta_vis',  'p_{#zeta}^{vis} [GeV]',        cont(0, 300)),
        ('pzeta_miss', 'p_{#zeta}^{miss} [GeV]',       cont(-150, 300)),
        ('mt_tot',     'm_{T}^{tot} [GeV]',            cont(0, 400)),
        ('mt2_ll',     'M_{T2}(lep,#tau) [GeV]',       cont(0, 150)),
    ]
...
def common_defines(df):
    ...
    # 1tau1l FakeLep: tau leg pinned at the analysis WP on BOTH legs; the
    # tight/anti-ID split lives on the LEPTON (Fakeable WP pool).
    df = df.Define('is_tight',
                   'lep1passAnalysisWP == 1 && tau1idDeepTau2017v2p1VSjet >= 8')
    df = df.Define('is_loose_not_tight',
                   'lep1passFakeableWP == 1 && lep1passAnalysisWP == 0 && '
                   'tau1idDeepTau2017v2p1VSjet >= 8')
    if FR_AST:
    ...
    # FakeLep transfer factor FF = fr/(1-fr) from the embedded 2017 map.
    # Applied on the anti-ID leg only (data_l / mc_l), where the lepton always
    # fails tight -> cone-pT = c*pt*(1+jetRelIso) unconditionally.
    df = df.Define('lepFR_w',      'lepff(lep1Id, lep1Pt, lep1Eta, lep1jetRelIso, 0)')
    df = df.Define('lepFR_w_up',   'lepff(lep1Id, lep1Pt, lep1Eta, lep1jetRelIso, 1)')
    df = df.Define('lepFR_w_down', 'lepff(lep1Id, lep1Pt, lep1Eta, lep1jetRelIso, -1)')
    if FAKETAU:
        # tau anti-ID leg (lep tight) and double-anti-ID leg
        df = df.Define('is_tau_antiid',
                       'lep1passAnalysisWP == 1 && tau1idDeepTau2017v2p1VSjet >= 2 && '
                       'tau1idDeepTau2017v2p1VSjet < 8')
        df = df.Define('is_both_antiid',
                       'lep1passFakeableWP == 1 && lep1passAnalysisWP == 0 && '
                       'tau1idDeepTau2017v2p1VSjet >= 2 && tau1idDeepTau2017v2p1VSjet < 8')
        # FR2D_YVAR: 'dfb' (default) or 'abseta' (eta-axis maps, tttt-style)
        _yv = ('tau1jetDeepFlavB' if os.environ.get('FR2D_YVAR', 'dfb') == 'dfb'
               else '(float)std::abs(tau1Eta)')
        if USE_MUFFIN:
            _ma = ('tau1decayMode, (tau1jetPt/std::max(tau1Pt,1.e-6f)), tau1Pt, '
                   'nsmalljets, nbtags, tau1Eta, tau1Phi')
            df = df.Define('tauFR_w',      'muffin_weight(%s)' % _ma)
            df = df.Define('_muf_rms',     'muffin_weight_rms(%s)' % _ma)
            df = df.Define('tauFR_w_up',   'tauFR_w + _muf_rms')
            df = df.Define('tauFR_w_down', 'std::max(0., tauFR_w - _muf_rms)')
        else:
            df = df.Define('tauFR_w',      'tauFR_weight_2d(tau1jetPt, %s, tau1decayMode, nsmalljets)' % _yv)
            df = df.Define('tauFR_w_up',   'tauFR_weight_2d_up(tau1jetPt, %s, tau1decayMode, nsmalljets)' % _yv)
            df = df.Define('tauFR_w_down', 'tauFR_weight_2d_down(tau1jetPt, %s, tau1decayMode, nsmalljets)' % _yv)
        df = df.Define('both_w', 'lepFR_w * tauFR_w')
    return df
...
def add_mc_weight(df, with_ttbb_kill=True, scale=1.0):
    ...
    # 1tau1l: leptonic OR-formula trigger SF; no triggerLumiSF (full-lumi paths).
    _trig = '' if NO_TRIGSF else ' * triggerSF_1tau1l_v1'
    ...
    # prompt LEPTON (1 prompt, 15 tau-decay, 22 conversion -> measurement-side
    # convention: conversions subtracted as prompt-like / folded into MC groups)
    df = df.Define('is_prompt_lep',
                   'lep1genPartFlav == 1 || lep1genPartFlav == 15 || lep1genPartFlav == 22')
    df = df.Define('mc_fr_w',      'mc_weight * lepFR_w')
    df = df.Define('mc_fr_w_up',   'mc_weight * lepFR_w_up')
    df = df.Define('mc_fr_w_down', 'mc_weight * lepFR_w_down')
    if FAKETAU:
        df = df.Define('mc_ftau_w',      'mc_weight * tauFR_w')
        df = df.Define('mc_ftau_w_up',   'mc_weight * tauFR_w_up')
        df = df.Define('mc_ftau_w_down', 'mc_weight * tauFR_w_down')
        df = df.Define('mc_both_w',      'mc_weight * both_w')
    return df
...
def get_regions():
    _all_regions = {
        ...
        'inclusive': ('inclusive_1tau1l', '1', '1#tau_{h}1l (FakeLep validation)'),
    }
    if CLOSURE_REGION:
...
# =========================================================================
def declare_lepff():
    """JIT-declare lepff(id, pt, eta, jri, shift) = FF = fr/(1-fr) from the 2017
    mT-template-fit lepton FR map (lepfr_map_2017_final.npz, 2026-08-07;
    regenerate values with lepfr_plot_maps.py). shift: 0 nominal, +1/-1 map err.
    Anti-ID leg only -> cone-pT = c*pt*(1+jetRelIso) unconditionally."""
    import ROOT
    ROOT.gInterpreter.Declare("""
float lepff(float lepid, float pt, float eta, float jri, int shift){
    static const float PT_E[6]  = {10.f, 15.f, 20.f, 30.f, 45.f, 100.f};
    static const float ETA_MU[4] = {0.f, 1.2f, 2.1f, 2.4f};
    static const float ETA_EL[4] = {0.f, 0.8f, 1.479f, 2.5f};
    static const float FR_MU[5][3] = {{0.5648f, 0.6216f, 0.6582f}, {0.7047f, 0.7763f, 0.7985f},
        {0.6648f, 0.7322f, 0.7529f}, {0.5905f, 0.6408f, 0.6717f}, {0.6169f, 0.6334f, 0.6176f}};
    static const float ER_MU[5][3] = {{0.0105f, 0.0153f, 0.0346f}, {0.0109f, 0.0144f, 0.0301f},
        {0.0070f, 0.0097f, 0.0214f}, {0.0135f, 0.0188f, 0.0407f}, {0.0381f, 0.0542f, 0.1114f}};
    static const float FR_EL[5][3] = {{0.3082f, 0.2881f, 0.2686f}, {0.2360f, 0.2400f, 0.2352f},
        {0.2422f, 0.2351f, 0.2375f}, {0.0689f, 0.0755f, 0.0647f}, {0.0566f, 0.0346f, 0.0341f}};
    static const float ER_EL[5][3] = {{0.0105f, 0.0144f, 0.0161f}, {0.0087f, 0.0099f, 0.0103f},
        {0.0068f, 0.0067f, 0.0066f}, {0.0039f, 0.0036f, 0.0028f}, {0.0086f, 0.0062f, 0.0033f}};
    const bool ismu = (std::abs((int)lepid) == 13);
    const float c = ismu ? 0.756f : 0.80f;
    float cpt = c * pt * (1.f + jri);
    if (cpt < 10.f)  cpt = 10.f;
    if (cpt > 99.9f) cpt = 99.9f;
    float aeta = std::abs(eta);
    const float* etae = ismu ? ETA_MU : ETA_EL;
    if (aeta > etae[3] - 1e-4f) aeta = etae[3] - 1e-4f;
    int ip = 4, ie = 2;
    for (int i = 0; i < 5; ++i) if (cpt  >= PT_E[i] && cpt  < PT_E[i+1]) { ip = i; break; }
    for (int j = 0; j < 3; ++j) if (aeta >= etae[j] && aeta < etae[j+1]) { ie = j; break; }
    float fr = ismu ? FR_MU[ip][ie] : FR_EL[ip][ie];
    if (shift != 0) fr += shift * (ismu ? ER_MU[ip][ie] : ER_EL[ip][ie]);
    if (fr < 0.f)    fr = 0.f;
    if (fr > 0.95f)  fr = 0.95f;
    return fr / (1.f - fr);
}
""")
...
def main():
    ...
    if FAKETAU:
        declare_fr2d()
        if USE_MUFFIN:
            with open(MUFFIN_HEADER) as _fh:
                if not ROOT.gInterpreter.Declare(_fh.read()):
                    raise RuntimeError('muffin_weight Declare failed')
            print('  USE_MUFFIN=1 -> fake-tau transfer factor from %s' % MUFFIN_HEADER)
        print('  FAKETAU=1 -> FakeTau from 1tau0l fr2d map (AS-IS); OUTDIR=%s' % OUTDIR)
    ...
    # 1tau1l data = SingleMuon (mu channel) + SingleElectron (e channel), routed
    # by lep1 flavour so an event can only be counted from its own PD.
    df_data_mu, data_files_mu = make_df('data', include_patterns=('SingleMuon',))
    df_data_el, data_files_el = make_df('data', include_patterns=('SingleElectron',))
    data_files = data_files_mu + data_files_el
    df_mc,   mc_files   = make_df('mc',   exclude_patterns=('FakeTau_',))

    print('=== Data files: %d (SingleMuon %d + SingleElectron %d) ==='
          % (len(data_files), len(data_files_mu), len(data_files_el)))
    print('=== MC files:   %d ===' % len(mc_files))

    df_data_list = []
    if df_data_mu is not None:
        df_data_list.append(common_defines(df_data_mu).Filter('abs(lep1Id) == 13'))
    if df_data_el is not None:
        df_data_list.append(common_defines(df_data_el).Filter('abs(lep1Id) == 11'))
    df_data = df_data_list[0]  # branch-availability probe only
    ...
    for reg_name, reg_sel, reg_label in closure_regions:
        ...
        # 1tau1l: tight/anti-ID split is entirely in is_tight/is_loose_not_tight
        # (WP-consistent with the FakeLep map by construction).
        sel_tight = '(%s) && %s && (%s)' % (reg_sel, REGION_BASE, EXTRA_CUT)
        sel_loose = '(%s) && %s && is_loose_not_tight && (%s)' % (reg_sel, REGION_BASE, EXTRA_CUT)

        df_data_t = [d.Filter(sel_tight + ' && is_tight') for d in df_data_list]
        df_data_l = [d.Filter(sel_loose) for d in df_data_list]
        _mcgate = ' && is_prompt_lep' + (' && tau1genPartFlav == 5' if FAKETAU else '')
        df_mc_l   = [d.Filter(sel_loose + _mcgate) for d in df_mc_list]
        if FAKETAU:
            sel_taul = '(%s) && %s && is_tau_antiid && (%s)' % (reg_sel, REGION_BASE, EXTRA_CUT)
            sel_both = '(%s) && %s && is_both_antiid && (%s)' % (reg_sel, REGION_BASE, EXTRA_CUT)
            df_data_ftau = [d.Filter(sel_taul) for d in df_data_list]
            df_mc_ftau   = [d.Filter(sel_taul + _mcgate) for d in df_mc_list]
            df_data_both = [d.Filter(sel_both) for d in df_data_list]
            df_mc_both   = [d.Filter(sel_both + _mcgate) for d in df_mc_list]
        else:
            df_data_ftau = df_mc_ftau = df_data_both = df_mc_both = []

        # Per-group tight-prompt MC filters
        df_grp_t = {g: grp_df[g].Filter(sel_tight + ' && is_tight'
                                        + ('' if PURE_MC else _mcgate))
                    for g in grp_df}
        ...
        for var_name, var_title, edges in plot_vars:
            ...
            if var_name in NO_ZERO_FILTER_VARS:
                ...
                ddft, dmft = df_data_ftau, df_mc_ftau
                ddb, dmb = df_data_both, df_mc_both
                dgrp = df_grp_t
                doverlay = df_overlay_t
            else:
                zf = '%s != 0' % var_name
                ddt = [d.Filter(zf) for d in df_data_t]
                ddl = [d.Filter(zf) for d in df_data_l]
                dml = [d.Filter(zf) for d in df_mc_l]
                ddft = [d.Filter(zf) for d in df_data_ftau]
                dmft = [d.Filter(zf) for d in df_mc_ftau]
                ddb = [d.Filter(zf) for d in df_data_both]
                dmb = [d.Filter(zf) for d in df_mc_both]
            ...
            entry = dict(
                ...
                data_t      =[d.Histo1D((hpfx + ('_data_t_%d' % k),      '', nbins, edges), fill_col)                for k, d in enumerate(ddt)],
                data_l      =[d.Histo1D((hpfx + ('_data_l_%d' % k),      '', nbins, edges), fill_col, 'lepFR_w')      for k, d in enumerate(ddl)],
                data_l_up   =[d.Histo1D((hpfx + ('_data_l_up_%d' % k),   '', nbins, edges), fill_col, 'lepFR_w_up')   for k, d in enumerate(ddl)],
                data_l_down =[d.Histo1D((hpfx + ('_data_l_down_%d' % k), '', nbins, edges), fill_col, 'lepFR_w_down') for k, d in enumerate(ddl)],
                ...
                grp_t       ={},
                overlays    =[],
                data_ftau     =[d.Histo1D((hpfx + ('_data_ftau_%d' % k),     '', nbins, edges), fill_col, 'tauFR_w')      for k, d in enumerate(ddft)],
                data_ftau_up  =[d.Histo1D((hpfx + ('_data_ftau_up_%d' % k),  '', nbins, edges), fill_col, 'tauFR_w_up')   for k, d in enumerate(ddft)],
                data_ftau_down=[d.Histo1D((hpfx + ('_data_ftau_dn_%d' % k),  '', nbins, edges), fill_col, 'tauFR_w_down') for k, d in enumerate(ddft)],
                mc_ftau       =[d.Histo1D((hpfx + ('_mc_ftau_%d' % k),       '', nbins, edges), fill_col, 'mc_ftau_w')      for k, d in enumerate(dmft)],
                mc_ftau_up    =[d.Histo1D((hpfx + ('_mc_ftau_up_%d' % k),    '', nbins, edges), fill_col, 'mc_ftau_w_up')   for k, d in enumerate(dmft)],
                mc_ftau_down  =[d.Histo1D((hpfx + ('_mc_ftau_dn_%d' % k),    '', nbins, edges), fill_col, 'mc_ftau_w_down') for k, d in enumerate(dmft)],
                data_both     =[d.Histo1D((hpfx + ('_data_both_%d' % k),     '', nbins, edges), fill_col, 'both_w')    for k, d in enumerate(ddb)],
                mc_both       =[d.Histo1D((hpfx + ('_mc_both_%d' % k),       '', nbins, edges), fill_col, 'mc_both_w') for k, d in enumerate(dmb)],
            )
            for g in dgrp:
    ...
    root_out_name = 'agreement_v29pre_1tau1l_fakelep'
    if args.variable is not None:
    ...
    for reg_name, reg_sel, reg_label in closure_regions:
        ...
        for var_name, var_title, edges in plot_vars:
            ...
            def hpfx_tmp(tag):
                return '%s_%s_tmp_%s' % (reg_name, var_name, tag)

            # Sum helper (also used for the per-PD data lists)
            ...
            # Data tight (SingleMuon + SingleElectron, flavour-routed)
            h_data = _sum_list(bk['data_t'], '%s_%s_data' % (reg_name, var_name))
            ...
            # aggregate loose-prompt MC split across inclusive-TT / rest dfs
            ...
            # FakeLep template = data_AR x FF - prompt-lep MC_AR x FF
            h_fake = _sum_list(bk['data_l'], '%s_%s_fakelep' % (reg_name, var_name))
            h_fake.Add(h_mc_l, -1.0)
            h_fake_up = _sum_list(bk['data_l_up'], '%s_%s_fakelep_up' % (reg_name, var_name))
            h_fake_up.Add(h_mc_l_up, -1.0)
            h_fake_dn = _sum_list(bk['data_l_down'], '%s_%s_fakelep_dn' % (reg_name, var_name))
            h_fake_dn.Add(h_mc_l_dn, -1.0)

            # FakeTau (net of the double-anti-ID overlap term); zero when FAKETAU=0
            if FAKETAU:
                h_ftau = _sum_list(bk['data_ftau'], '%s_%s_faketau' % (reg_name, var_name))
                h_ftau.Add(_sum_list(bk['mc_ftau'], hpfx_tmp('mcft')), -1.0)
                h_ftau_up = _sum_list(bk['data_ftau_up'], '%s_%s_faketau_up' % (reg_name, var_name))
                h_ftau_up.Add(_sum_list(bk['mc_ftau_up'], hpfx_tmp('mcftu')), -1.0)
                h_ftau_dn = _sum_list(bk['data_ftau_down'], '%s_%s_faketau_dn' % (reg_name, var_name))
                h_ftau_dn.Add(_sum_list(bk['mc_ftau_down'], hpfx_tmp('mcftd')), -1.0)
                h_both = _sum_list(bk['data_both'], hpfx_tmp('both'))
                _mcb = _sum_list(bk['mc_both'], hpfx_tmp('mcb'))
                if _mcb is not None:
                    h_both.Add(_mcb, -1.0)
                for _h in (h_ftau, h_ftau_up, h_ftau_dn):
                    _h.Add(h_both, -1.0)
            else:
                h_ftau = h_fake.Clone('%s_%s_faketau' % (reg_name, var_name)); h_ftau.Reset()
                h_ftau_up = h_ftau.Clone('%s_%s_faketau_up' % (reg_name, var_name))
                h_ftau_dn = h_ftau.Clone('%s_%s_faketau_dn' % (reg_name, var_name))
            h_ftau.SetFillColor(ROOT.kOrange - 2); h_ftau.SetLineColor(ROOT.kBlack); h_ftau.SetLineWidth(1)
            if PURE_MC:
            ...
            if var_name == 'ht':
                _nb = h_data.GetNbinsX() + 1
                tot_p = h_mc_p_total.Integral(0, _nb) if h_mc_p_total is not None else 0.0
                tot_pred = tot_p + h_fake.Integral(0, _nb) + h_ftau.Integral(0, _nb)
                print('  Data tight:        %.0f' % h_data.Integral(0, _nb))
                print('  MC prompt total:   %.1f' % tot_p)
                print('  FakeLep (data-MC): %.1f' % h_fake.Integral(0, _nb))
                if FAKETAU:
                    print('  FakeTau (net):     %.1f' % h_ftau.Integral(0, _nb))
                print('  Pred (MC+Fake):    %.1f' % tot_pred)
                if tot_pred > 0:
                    print('  Data/Pred:         %.3f' % (h_data.Integral(0, _nb) / tot_pred))
            ...
            h_tot.Add(h_ftau); h_tot_up.Add(h_ftau_up); h_tot_dn.Add(h_ftau_dn)
            ...
            if not PURE_MC:
                leg.AddEntry(h_fake, 'Fake lep (FR method)', 'f')
                if FAKETAU:
                    leg.AddEntry(h_ftau, 'Fake #tau (1#tau0l map)', 'f')
            ...
            lt.DrawLatex(0.15, 0.83, reg_label + (' [pure MC]' if PURE_MC else ''))
            ...
            fout.cd()
            h_data.Write(); h_fake.Write(); h_ftau.Write(); h_tot.Write(); h_ratio.Write()
